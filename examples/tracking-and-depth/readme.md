...
While the pointcloud is generated by the depth camera of a moving camera set but it is displayed using a static point of view.

## Expected Output
The application should open a window with a pointcloud visualized in 3D.
A static scene should appear on the screen regardless the motion of the camera set.
This allows "scanning" the scene (frame-by-frame) by moving the camera set. (Currently, no depth data is accumulated or filtered over time.)
The 3D display of the pointcloud can be rotated or zoomed using the mouse.
...
Please note that the homogeneous transformation is in a row-major 3x4 matrix of the form `H = [R, t]` where `R` is the rotation matrix and `t` the translation vector of the depth, i.e. infrared 1, frame, with respect to the T265 body frame (see above).
